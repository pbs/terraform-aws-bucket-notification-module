...
module "bucket-notification" {
    source = "github.com/pbs/terraform-bucket-notification-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/bucket-notification'
gh -R 'pbs/terraform-bucket-notification-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/bucket-notification'
rm -f release.tar.gz
```
...
```bash
git remote add -f bucket-notification git@github.com:pbs/terraform-bucket-notification-module.git
git subtree add --prefix terraform/modules/bucket-notification bucket-notification main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/bucket-notification bucket-notification $REF --squash
```
